...
public class Inflector {
	...
	public Inflector() {
		...
		uncountables = Arrays.asList("equipment", "information", "rice",
	 ...
	 *            set to true if first character needs to be capitalized, false
	 *            if not.
	 * @return camel case version of underscore.
	 */
	public String camelize(String underscore, boolean capitalizeFirstChar) {
		...
		return capitalizeFirstChar ? result : result.substring(0, 1)
	...
	/**
	 * If a table name is made of two other table names (as is typical for many
	 * to many relationships), this method retrieves a name of "another" table
	 * from a join table name. For instance, if a source table is "payer" and
	 * the target is "player_game", then the returned value will be "game".
	 * 
	 * @param source
	 *            known table name. It may or may not exist in the target table
	 *            name.
	 * @param target
	...
	public String getOtherName(String source, String target) {
		...
		if (target.contains(source) && !target.equals(source)) {
			...
			other = start == 0 ? target.substring(source.length()) : target
					.substring(0, start);
		} else {
			return null;
		}
